...
- **Known issue — the verify pass aborts a few minutes in.** The run stops early with `tunnel error: namespace 'wlvpn' lost live egress after N verified book(s)`, observed at N=15, 17 and 7 across separate runs, between ~2 and ~3½ minutes after the namespace was built. Verified rows are kept and a re-run resumes, so each attempt banks a handful of books, but a full ~400-book pass never completes unattended and freshly fetched deals stay `deal_status IS NULL` — invisible on the tab, since `current_deals()` lists only `current` rows.

  What the evidence says (2026-09-01):

  - **The tunnel is fine when idle.** A namespace left alone held one exit IP for a 10-minute test with WireGuard handshakes rekeying normally every ~2 minutes.
  - **The probe endpoint is not rate-limiting.** 20 consecutive `api.ipify.org` probes through the tunnel all returned the IP.
  - **It is not a momentary blip.** `_tunnel_live` already retries across a ~60-second window (`VERIFY_TUNNEL_RETRIES` / `VERIFY_TUNNEL_RETRY_DELAY`) before giving up, so egress must be continuously unusable for over a minute to trigger the abort.
  - **Neither book count nor elapsed time predicts it.** Only the presence of Chromium traffic does.

  So the failure needs browser load in the namespace to appear, and it is a real loss of usable egress rather than a flaky check. When diagnosing, first separate **DNS** from **transport** — the namespace resolves through Nord's resolver (`/etc/netns/wlvpn/resolv.conf`, default `103.86.96.100`), and a resolver that stops answering under Chromium's query volume looks identical to a dead tunnel from `tunnel_egress_ip()`'s point of view. Note that `tunnel_egress_ip` logs curl's exit code at **`log.debug`** while the verifier runs at INFO, so the one fact that distinguishes the two (rc 6 = DNS, rc 7/28 = transport) is discarded on every failure; raise that to a warning before chasing anything else.
