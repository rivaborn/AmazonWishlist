...
- `GET /api/sync/deals` — the whole BookBub `deal` table (all columns, incl. `cover`/`description`), the whole `hidden_book` table (`hidden_books` in the payload — the per-book hides the tab filters on; without them the mirror would list every book the primary dismissed), plus the cover images as base64, fetched on the same once-a-day pull. The table is small, so the secondary **replaces its local rows wholesale** (no watermark, no cursor) and rewrites the covers into its own `DEALS_COVERS_DIR` — a secondary duplicates the primary's BookBub Deals page, covers and tooltips included.
...
| `owned_in_grimmory` | `1` owned / `0` not owned / **NULL when `grimmory.db` is unavailable**. An approximate normalised title+author match against `data/grimmory.db` — kept in the DB so a human can audit its accuracy. The author must be **equal**, in either the name as written or with a single `Surname, Given` comma flipped (see below). |
...
| `hidden` | **Legacy, no longer read or written.** Was the per-row hide flag; hides now live in `hidden_book` (below). The column stays so a peer running older code still gets the field it inserts, and its `1`s are migrated into `hidden_book` once, on the first run after the upgrade. |
...
**Author spelling (`deals_db._author_keys`).** Grimmory inherits a large minority of its authors from Calibre surname-first (`Matheson, Richard`) while BookBub always writes them out (`Richard Matheson`), so an equal-author test alone silently failed on every one of them and the owned book stayed on the tab. Each author is therefore indexed and looked up under two keys: the name as written, and the same name with a single `Surname, Given` comma flipped. The flip is additive and tried on both sides, because either source can hold the inverted spelling. A **single** comma with two non-empty halves is the only shape treated as an inversion: a BookBub credit can also use a comma to separate co-authors (`Jennifer Reingold, Daniel Reingold`), and flipping that yields a key matching nothing rather than a false positive. Measured on the live data (2026-09-06): 1,615 of 37,771 catalog rows are stored surname-first, and the flip recognised 14 of 688 deal rows as owned that the equal-author test alone had missed, with none lost.

The `hidden_book` table holds the books dismissed with the tab's **Hide** checkbox — one entry per **book**, not per deal row:

| column      | meaning                                                                                                                                                                        |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `book_key`  | primary key. Either `asin:<ASIN>` (from the row's `amazon_url`) or `meta:<normalised title>\|<normalised author>`. Hiding a book writes **both** keys; un-hiding deletes both. |
| `title`     | the title as it read when the book was hidden — audit only, nothing matches on it.                                                                                             |
| `author`    | the author as it read when the book was hidden — audit only.                                                                                                                   |
| `hidden_at` | ISO timestamp of when the book was first hidden.                                                                                                                               |

Why a separate table, and why two keys: BookBub re-features the same book on later dates, each date is its own `deal` row, and step 1c of the daily updater (`deduplicate()`) deletes all but the newest row per book — so a hide stored on the row was deleted with it, and the deal came back on the tab the next morning. Keying by book identity (the same identity `deduplicate()` groups on) makes the hide outlive both the new row and the dedup, and writing the ASIN key beside the title+author key covers a deal whose `amazon_url` is an unresolved BookBub intermediate link on one date and the canonical product URL on the next. `deals_db.current_deals()`, `pending_deals()` and `recheck_deals()` all filter against this table, so a hidden book is neither shown nor re-verified. The legacy `deal.hidden` flags are migrated into it once, by the first `ensure_schema()` call that finds the table missing.
...
1. **Fetch + store** — pulls the day's BookBub daily deals (`bookbub.fetch_daily_deals`: logs into the BookBub account with `BOOKBUB_USERNAME` / `BOOKBUB_PASSWORD`; Playwright clears the Cloudflare challenge) and upserts them into `data/deals.db` via `deals_db.store_deals` — idempotent per date, computes `owned_in_grimmory` from `data/grimmory.db` (books you own never show on the tab) and sets `no_amazon_link`. It then **rebuilds `data/grimmory.db` from the Grimmory server** (`build_grimmory_db.build` over `GRIMMORY_LIBRARIES`) and **refreshes `owned_in_grimmory` on every stored row** against the rebuilt file (`deals_db.refresh_owned`) — so a book added to your Grimmory libraries since the last run is re-flagged owned and drops off the tab on the next run without any re-fetch; a missing/empty `grimmory.db` leaves the flags NULL and the run still completes. The rebuild is **best-effort**: absent `GRIMMORY_USERNAME` / `GRIMMORY_PASSWORD`, an unreachable Grimmory, or a DB error is logged and the run continues against the catalog already on disk (the builder stages into a scratch table and renames it in one transaction, so a failed rebuild leaves the previous catalog whole). `--no-catalog-refresh` skips it. Before this step the catalog was refreshed only by the **monthly** "Update Owned Books" job or by hand, so a book bought on the 2nd kept showing as a deal for the rest of the month; that monthly job still runs, because it also moves owned wishlist books to Purchased. It then **dedups** the stored rows (`deals_db.deduplicate`) so a book re-featured on a later date (same identity — ASIN, else normalised title+author) collapses to the single most recent deal per book; best-effort like the owned refresh — a failure is logged and the run continues.
2. **Re-verify** — then runs `scripts/verify_deals.py --recheck` (the tunnel/fingerprint/pacing loop above) over every deal that is neither `expired` nor `hidden` — i.e. `current`, `unknown`, and unchecked rows still visible on the tab — re-reading each against its live Amazon price. Deals that come back `current` flow into the BookBub Deals tab automatically.
   ...
   - `hidden` — dismissed by the user (a `hidden_book` match), and `current_deals()` filters hidden books off the tab regardless of status. These dominate in practice: on 2026-09-01 they were **271 of 390** recheckable rows, so excluding them cut the nightly pass to **118 books** — roughly a third of the Amazon reads, and a correspondingly smaller anti-bot footprint. The trade-off is that un-hiding a deal shows whatever status it carried when it was hidden, because nothing refreshed it meanwhile.
...
- `data/grimmory.db` (`GRIMMORY_DB`) must be present for owned detection; without it, `owned_in_grimmory` stays NULL (unavailable) and those deals still show. The daily run rebuilds it first when `GRIMMORY_USERNAME` / `GRIMMORY_PASSWORD` are set (step 1 above), so on a configured host the file is recreated even if it was deleted.
- `--check` is a dry run: it prints the date, the credential status (`credentials: set` / `no BOOKBUB_USERNAME / BOOKBUB_PASSWORD configured` — never the values), the recheckable (neither expired nor hidden) deal count read straight from the DB, and the catalog state (path, build time, and whether the nightly rebuild is enabled) — no network, browser, or tunnel.
