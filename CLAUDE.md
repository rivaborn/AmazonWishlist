...
### "Update Owned Books" progress (`owned_update.py`)

The Settings tab's button and the monthly cron share `trigger_owned_update()`, which runs the operation in a daemon thread behind a lock and narrates it into the same `_STATUS` dict the page reads — `GET /api/owned/status` is that dict, polled once a second for the bar and the activity window (`app/templates/settings.html`). Three rules keep the bar honest:

- **`step` counts *completed* steps and `_phase()` banks the previous one on entry**, so each step needs exactly one call and the count can never run ahead of the work; `_run()` sets `step = total_steps` on success, which is what lands the bar on 100%. `_total_steps()` is derived from `GRIMMORY_LIBRARIES` (5 fixed steps + one per library), so the bar is **determinate from the first paint** — add or remove a `_phase()` call and you must move that number with it, or the bar over/undershoots.
- **The long middle of the run is not ours.** `build_grimmory_db.build()` takes an optional `on_progress(kind, msg)` (`"phase"` advances, `"note"` only logs) which `_relay` adapts; it is the only reason the per-library fetches show up at all. Both CLI callers leave it unset, so the script's behaviour is unchanged.
- **The log is capped and drops the oldest lines**, so a client must redraw it rather than append by index — and `owned_update_status()` hands back a *copy* of the list, because the worker appends to it while the response is being serialised.

Unlike the scrape there is **no on-disk mirror and no resume**: a run is minutes, so a restart mid-run just loses the commentary. Don't add one without also deciding what a half-applied run means — `_mark_owned_purchased()` has already committed by the time the deals refresh runs.
